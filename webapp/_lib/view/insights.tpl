...
{foreach from=$insights key=tid item=i name=insights}
  ...
  {if $i->instance->network eq 'twitter'}
    {capture name="share_link" assign="share_link"}
      <a class="share-link {$i->instance->network}" href="https://twitter.com/intent/tweet?related=thinkup&amp;text={$i->headline|strip_tags:true|strip|truncate:100}&amp;url={$permalink|html_entity_decode|escape:'url'}&amp;via=thinkup">Tweet this</a>
    {/capture}
  {elseif $i->instance->network eq 'facebook'}
    {capture name="share_link" assign="share_link"}
      <a class="share-link {$i->instance->network}" href="https://www.facebook.com/sharer.php?u={$permalink|html_entity_decode|escape:'url'}">Share on Facebook</a>
    {/capture}
  {/if}
